    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.result) {
                ...
                var groupListHTML = "";
                var groupList = data.groupInfoList;
...
function setSentMessageMap(total) {
    sentMessageMap = new SentMessageMap();
    for (var i = 0; i < total; i++) {
        sentMessageMap.put(i, new Array());
    }
}
...
$(".myfile").on("fileuploaded", function (event, data, previewId, index) {
    ...
    console.log("afterfileuploaded", data);
    setTimeout(function () {
...
$('.myfile').on('filepreupload', function (event, data, previewId, index) {
    console.log("before filepreupload", data);
});
...
$('.sendBtn').on('click', function () {
    ...
    if (news == '') {
        ...
        if (toUserId.length != 0) {
            console.log("发送者： ", fromUserId, " 接收者： ", toUserId, " 私聊内容为： ", news)
            ws.singleSend(fromUserId, toUserId, news);
        } else {
            console.log("群发内容为： ", news)
